...
characterized IP pool (2nd phase), to finally generate the winning
architecture and mapping candidate of the DNN implementation (in
JSON format).
...
Figure~\ref{fig:talos-input-output}. Talos returns a list of
implemented architectures, not a single design: each entry pairs a
...
mapping of the workload on it. Preserving the list instead of only
the best candidate is what exposes the trade-offs between
alternatives.
