...
iterative structure. The global view of the Talos EDA tool developed
is shown in Figure~\ref{fig:talos-input-output}. Talos takes the 
file describing the DNN (in ONNX format), an processes it in two
levels (first technology independent + second technology dependent) in
which the user must add their PPA constraints (1st \& 2nd), provided a
characterized IP pool (2nd phase), to finally generate the ranking of 
architectures and mapping candidates of the DNN implementation (in
JSON format).
...
\begin{figure}[htbp]
  \centering
  \resizebox{\linewidth}{!}{\input{img/input_output_tikz}}
...
\item in Level~1, generate and evaluate abstract accelerator
architectures and preserve the workload activity obtained for the
selected candidates;
\item in Level~2, convert those candidates into abstract components and
match them with compatible physical IP implementations;
\item in Level~2, reject implementations that cannot satisfy the
required capacity, bandwidth, throughput, frequency, or user
constraints;
\item in Level~2, estimate workload latency, energy, average power,
area, and timing, while reporting the external DRAM contribution
separately;
\item support configurable optimization objectives, genetic exploration
in both levels, and exhaustive exploration in Level~2; and
\item return feasible alternatives together with their metrics,
constraint status, and the mapping generated for each
selected architecture, exported as JSON, instead of hiding the
trade-offs behind one design.
\end{itemize}

The ONNX workload is a central input of the flow as seen in Figure~\ref{fig:talos-input-output}. 
Talos reads the operand precision directly
from the element data type (\texttt{elem\_type}) declared for each
tensor in the ONNX graph, while the graph structure and the tensor
shapes define the layers to be mapped. With this information, Level~1
generates the mapping of the workload on each candidate architecture
and estimates its cycles and memory accesses. The same precision is
later used to select compatible processing elements in Level~2.
...
The output is the last element of
Figure~\ref{fig:talos-input-output}. Talos returns a ranked list of architectures, not a single design: each entry pairs a
Level~1 architecture with the IPs selected for its components,
together with its estimated metrics, its constraint status, and the
mapping of the workload on it. The list is ordered from best to worst
according to the user-selected objectives: by the objective itself
when only one is active, and by the global balanced score
(Section~\ref{subsec:feasibility_ranking}) when several are combined.
Throughout this document, the \emph{winning architecture} refers to
the top-ranked entry of this list; the term therefore reflects the
objectives chosen by the user, not a decision made by the tool.
Preserving the list instead of only the winner is what exposes the
...
implementation.
\item \textbf{HW Implementation.} This final phase corresponds to the final 
HW implementation of the obtained solution using standard EDA tools for chip design 
...
PE arrays, and dataflows, together with its performance evaluation. 
For the second level, the design was
...
terms used by ZigZag to describe an accelerator. In this scheme, each
memory declares which operands it stores. The three large memories are
the upper levels of the hierarchy: in the example of the figure,
Memory~1 is the highest level and holds the three operands (inputs,
weights, and outputs), while Memory~2 and Memory~3 are intermediate
buffers that keep only the operands assigned to them, closer to the
array. The small input and output memories are the lowest level: each
input memory stores the input operands consumed by the operational
units of its row, each output memory stores the outputs and partial
sums they produce, and the register file attached to each unit (RF~W
in the figure) holds its weights.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\linewidth]{img/abstract_arch.png}
  \caption{Abstract accelerator representation of a 4x4 PE array used to connect Level~1
...
genome (described in Subsection~\ref{subsec:genome}) and its decoder were 
implemented. Then, the generation of
...
Another example was the introduction of the objective adapter. At
first, ZigZag was embedded directly into the evaluation logic, so the
rest of the tool depended on its output format. The objective adapter
encapsulates ZigZag behind a fixed interface and translates its raw
evaluation results into the objectives used by the optimizer. If a
user wants to evaluate candidates with a different tool, only this
component has to be replaced, instead of the main exploration logic.
...
This validation covered the complete flow: the two-level structure and
the modular architecture of the tool, the integration of ZigZag as the
exploration engine of the first level, and the IP characterization
approach of the second level, exercised with the characterized pools of
Appendix~\ref{app:ip_pools} rather than with synthetic values.
...
mapped netlist. Third, the synthesized area, timing and power are
compared against the Level~2 estimates of the same configuration; this
comparison is the validation reported in
Section~\ref{sec:synthesis_validation}. During this investigation, the
TSMC 65~nm low-power library was selected to build the IP pool.
...
include a characterized library, before running Level~2, the user (or a future tool development) has
...
in a composite PE or the numerical format. MACs per cycle is the
sustained throughput of the PE and is unrelated to pipelining: a
pipelined PE still accepts one MAC per cycle, and its pipeline depth
only adds latency, which is stored separately. A value greater than
one appears only in composite PEs that physically contain more than
one MAC unit, such as the PE of OpenEye~\cite{openeye_repo}. This process is manual but
...
The development milestones introduced in Subsection~\ref{subsec:milestones} provide a
