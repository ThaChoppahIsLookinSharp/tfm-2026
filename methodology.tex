...
\todo[inline]{que implica el onnx, porque? precsion, layers, etc
en general input output, el output está escueto, no está claro el output
a lo mejor da pa una figura, inputs X e Y, output Z.
la characterización hecha por síntesis es relevante.
Que la tecnología está presente en el input es relevante.
}
...
%improve writting.
The inclusion of an ONNX workload is crucial in the process of the
design an application. This allow the tool
to infer the precision used for data representation, generate the
mapping for a generated architecture and estimate the cicles and 
memory accesses on said architecture on Level~1.

%improve writting.
The IP pool exposes the data that the Level~2 requires in order
to give an estimation of PPA.
...
\todo[inline]{Lista IP pool
ma o menos, tengo que decir  como he compilado las cosas y demas.
Decir que se podría automatizar también.}
...
The milestone \ref{milestone:m1} was fulfilled through the definition of the
...
The milestone \ref{milestone:m2} was fulfilled through the Level~1 genome,
...
The fourth milestone was fulfilled by separating the evaluators from the optimization objectives and by keeping the main parts of the flow as independent components. Workloads,
IP pools, constraints, and objective combinations can be changed without modifying the
complete exploration flow, and new objective functions can be added on top of the raw
evaluation metrics.
